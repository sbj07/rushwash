...
public class OrderDao {
   ...
   public List<OrderVo> getorderList(Connection conn, String memberNo , String deleteYn) throws Exception {
      ...
      String sql = "SELECT O.NO ,O.PRICE ,TO_CHAR(O.PAYMENT_DATE ,'YYYY\"년\"MM\"월\"DD\"일\"') AS PAYMEBT_DATE ,TO_CHAR(O.EXP_DATE ,'YYYY\"년\"MM\"월\"DD\"일\"') AS EXP_DATE ,T.STATUS AS LAUNDRY_STATUS ,M.NAME AS MEMBER_NAME ,M.TEL ,O.ADDRESS ,O.REQUEST FROM MEMBER M JOIN LAUNDRY_ORDER O ON M.NO = O.MEMBER_NO JOIN ORDER_STATUS T ON O.ORDER_STATUS = T.NO WHERE O.NO = '?' AND O.DEL_YN = '?'";
