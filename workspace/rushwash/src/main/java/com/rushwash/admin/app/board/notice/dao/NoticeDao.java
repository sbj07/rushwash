...
public class NoticeDao {
	//게시글 목록 조회
	   public List<NoticeVo> selectNoticeList(Connection conn, PageVo pvo) throws Exception{
	      ...
	      //SQL
	      String sql = "SELECT * FROM( SELECT ROWNUM RNUM, T.* FROM ( SELECT N.NO, N.TITLE , N.CONTENT , M.MANAGER_ID , N.ENROLL_DATE FROM NOTICE N JOIN MANAGER M ON N.MANAGER_NO = M.NO WHERE N.DEL_YN = 'N' ORDER BY NO ASC) T) WHERE RNUM BETWEEN ? AND ?";
